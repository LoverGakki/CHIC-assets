        ...
        if ($admin->username == 'admin') {
            ...
            if (!$admin->im_user_id) {
                ...
                if ($imRegister['code'] == 1) {
                    //获取token信息
                    $imTokenData = $imLogic->getUserToken($imRegister['data'], 'admin');
                    $admin->save([
                        'im_user_id' => $imRegister['data'],
                        'im_token' => $imTokenData['data'],
                    ]);
                    $admin->im_token = $imTokenData['data'];
                }
            } else {
                //获取token信息
                $imTokenData = $imLogic->getUserToken($admin->im_user_id, 'admin');
                $admin->save([
                    'im_token' => $imTokenData['data'],
                ]);
                $admin->im_token = $imTokenData['data'];
